...
subtitle: "Crosswalk"
description: >
  A comprehensive description of the internal mappings performed by **cffr**.
...
This vignette provides an explicit map between the metadata sources used by
**cffr** and each valid key in the [Citation File Format schema version
1.2.0](https://github.com/citation-file-format/citation-file-format/blob/main/schema-guide.md#valid-keys).

## Summary {#summary}

This section summarizes the keys that **cffr** can coerce and the source of
information for each one. Details for each key are presented in the next
section. The assessment is based on the [Guide to Citation File Format schema
version
1.2.0](https://github.com/citation-file-format/citation-file-format/blob/main/schema-guide.md#valid-keys)
...
This key is not extracted from package metadata. See its description in the
[Guide to CFF schema
...
it selects the first citation as the `"preferred-citation"` and the rest of them
