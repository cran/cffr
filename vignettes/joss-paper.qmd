...
Citation File Format is being adopted by GitHub as the primary format for its
...
**cffr** provides utilities to generate and validate `CITATION.cff` files for
**R** [@R_2021] packages by parsing the `DESCRIPTION` file and `inst/CITATION`.
The package also includes utilities and examples for parsing components such as
persons and additional citations, plus vignettes that illustrate basic package
usage and technical details about the metadata extraction process.
...
**cffr** allows **R** package developers to create `CITATION.cff` files from
...
assess the validity of the file against the Citation File Format schema.
